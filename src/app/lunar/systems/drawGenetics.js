...
const drawGenetics = (canvas, scene) => {
    ...
    last.maxFitness = scene.ships.sort((a,b) => a.fitness < b.fitness ? 1 : -1)[0].fitness    
    last.meanFitness = Math.round(scene.ships.reduce((sum, next) => sum + (next.fitness > 0 ? next.fitness : 0), 0) / scene.ships.length)
    const prev = simulation.stats.length > 0 ? simulation.stats[simulation.stats.length - 1] : false
    
    const texts = [
        'Generation: ' + last.generation,
        'Max Fitness: ' + Math.round(globalMaxes.maxFitness),
