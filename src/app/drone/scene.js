...
let currentTarget = new p5.Vector(
    config.center.x * 0.5,
    200
)
...
export const nextCurrentTarget = () => {
    ...
    currentTarget = new p5.Vector(nX, 200)//config.height - 20)
    currentStart = new p5.Vector(sX, config.height - 50)
    noiseSeed += 0.01
    startNoise += 0.0
}
...
const setupShips = () => {
    ...
    for(let i = 0 ; i < config.shipscount ; i++) {
        spawnShip(currentStart.x, currentStart.y)
    }
}
